...
class SymbolEditDialog(QDialog):
    def __init__(self, initialColor, initialSize, isWidth=False, parent=None):
        ...
        self.initUi(initialSize, isWidth)

    def initUi(self, size, isWidth):
        """Initialize UI components."""
        layout = QVBoxLayout(self)
        self.setupColorInput(layout)
        self.setupSizeInput(layout, size, isWidth)
        self.setupButtons(layout)

    def setupColorInput(self, layout):
        """Create color selection widgets."""
        hLayout = QHBoxLayout()
        hLayout.addWidget(QLabel(self.tr("Color:")))
    ...
    def setupSizeInput(self, layout, size, isWidth):
        """Create size/width input widgets."""
        label = self.tr("Width:") if isWidth else self.tr("Size:")
    ...
    def updateColorButton(self):
        """Apply color style to button."""
        c = self.currentColor
        style = f"QToolButton {{ background-color: rgb({c.red()}, {c.green()}, {c.blue()}); border: 1px solid #999; }}"
